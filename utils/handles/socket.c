...
int tls_socket_connect(mbedtls_ctx_t *tls_ctx, char const *host, uint16_t port,
  ...
  // init RNG
  if (mbedtls_ctr_drbg_seed(&tls_ctx->ctr_drbg, mbedtls_entropy_func,
                            &tls_ctx->entropy, (unsigned char const *)drgb_pres,
                            strlen(drgb_pres)) != 0) {
  ...
  // parsing CA
  if (ca_pem) {
    if (mbedtls_x509_crt_parse(&tls_ctx->cacert, (unsigned char *)ca_pem,
                               strlen(ca_pem) + 1) != 0) {
  ...
  if ((NULL != client_cert_pem) && (NULL != client_pk_pem)) {
    if (mbedtls_x509_crt_parse(&tls_ctx->client_cacert,
                               (unsigned char *)client_cert_pem,
                               strlen(client_cert_pem) + 1) != 0) {
      *error = RC_UTILS_SOCKET_TLS_CLIENT_PEM;
      return -1;
    }
    if (mbedtls_pk_parse_key(&tls_ctx->pk_ctx, (unsigned char *)client_pk_pem,
                             strlen(client_pk_pem) + 1, NULL, 0) != 0) {
  ...
  if (mbedtls_net_connect(&tls_ctx->net_ctx, host, port_string,
                          MBEDTLS_NET_PROTO_TCP) != 0) {
    *error = RC_UTILS_SOCKET_CONNECT;
    return -1;
  }
  // client setup
  if (mbedtls_ssl_config_defaults(&tls_ctx->ssl_conf, MBEDTLS_SSL_IS_CLIENT,
  ...
  mbedtls_ssl_conf_rng(&tls_ctx->ssl_conf, mbedtls_ctr_drbg_random,
                       &tls_ctx->ctr_drbg);
  ...
  // BIO callbacks
  mbedtls_ssl_set_bio(&tls_ctx->ssl, &tls_ctx->net_ctx, mbedtls_net_send,
                      mbedtls_net_recv, NULL);

  if (is_client_auth) {
    if (mbedtls_ssl_conf_own_cert(&tls_ctx->ssl_conf, &tls_ctx->client_cacert,
                                  &tls_ctx->pk_ctx) != 0) {
  ...
  while ((mbedtls_ret = mbedtls_ssl_handshake(&tls_ctx->ssl)) != 0) {
    if (mbedtls_ret != MBEDTLS_ERR_SSL_WANT_READ &&
