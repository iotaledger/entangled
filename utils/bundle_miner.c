...
uint64_t bundle_miner_mine(byte_t const *const bundle_normalized_max, uint8_t const security, trit_t *const essence,
  ...
  uint64_t index = trits_to_long(essence + OBSOLETE_TAG_OFFSET, OBSOLETE_TAG_LENGTH);
  ...
  for (size_t i = 0; i < procs; i++) {
    ...
    ctxs[i].essence = (trit_t *)malloc(sizeof(trit_t) * essence_length);
    memcpy(ctxs[i].essence, essence, essence_length);
    ctxs[i].essence_length = essence_length;
    ctxs[i].index = index + (i * (rounded_count / procs));
  ...
  for (size_t i = 0; i < procs; i++) {
    ...
    if (ctxs[i].probability < probability) {
      ...
      optimal_index = ctxs[i].optimal_index;
    }
    free(ctxs[i].essence);
  }
