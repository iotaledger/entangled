...
int main(void) {
  ...
  api.consensus->milestone_tracker.latest_milestone_index = LATEST_MILESTONE_INDEX;
  api.consensus->milestone_tracker.latest_solid_subtangle_milestone_index = LATEST_SOLID_MILESTONE_INDEX;
  api.consensus->milestone_tracker.milestone_start_index = MILESTONE_START_INDEX;
  ...
  flex_trits_from_trytes(latest_milestone_trits, NUM_TRITS_HASH, LATEST_MILESTONE, HASH_LENGTH_TRYTE,
                         HASH_LENGTH_TRYTE);
  memcpy(api.consensus->milestone_tracker.latest_milestone, latest_milestone_trits, FLEX_TRIT_SIZE_243);
  ...
  flex_trits_from_trytes(latest_solid_milestone_trits, NUM_TRITS_HASH, LATEST_SOLID_MILESTONE, HASH_LENGTH_TRYTE,
                         HASH_LENGTH_TRYTE);
  memcpy(api.consensus->milestone_tracker.latest_solid_subtangle_milestone, latest_solid_milestone_trits,
         FLEX_TRIT_SIZE_243);
  ...
  memcpy(api.consensus->milestone_tracker.coordinator, coordinator_trits, FLEX_TRIT_SIZE_243);
  // Adding neighbors
  ...
  TEST_ASSERT(request_transaction(&node.transaction_requester, &tangle, hashes[0], false) == RC_OK);
  TEST_ASSERT(request_transaction(&node.transaction_requester, &tangle, hashes[1], false) == RC_OK);
  TEST_ASSERT(request_transaction(&node.transaction_requester, &tangle, hashes[2], false) == RC_OK);
