...
void test_deserialize_find_transactions(void) {
  ...
  TEST_ASSERT_EQUAL_STRING(
      TEST_HASH3, find_transactions_res_hash_at(deserialize_find_tran, 2));
  TEST_ASSERT_EQUAL_STRING(
...
void test_deserialize_get_trytes(void) {
  ...
  serializer.vtable.get_trytes_deserialize_response(&serializer, json_text,
                                                    tips);
  TEST_ASSERT_EQUAL_STRING(TRANSACTION_TRYTES_1, get_trytes_res_at(tips, 0));
...
void test_deserialize_get_balances(void) {
  ...
  TEST_ASSERT_NULL(get_balances_res_milestone_at(deserialize_get_bal, 1));
...
void test_serialize_attach_to_tangle(void) {
  ...
  const char* json_text =
      ...
      "\",\"minWeightMagnitude\":18,\"trytes\":[\"" TRANSACTION_TRYTES_1
      "\",\"" TRANSACTION_TRYTES_2 "\"]}";
  ...
  attach_to_tangle_req_add_trytes(attach_req, TRANSACTION_TRYTES_1);
  attach_to_tangle_req_add_trytes(attach_req, TRANSACTION_TRYTES_2);
