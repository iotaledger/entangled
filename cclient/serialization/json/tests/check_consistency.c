...
static void test_request(void) {
  ...
  serializer.vtable.check_consistency_deserialize_request(
      &serializer, serializer_out->data, req_de);

  TEST_ASSERT_EQUAL_INT(hash243_queue_count(req->tails),
                        hash243_queue_count(req_de->tails));
  TEST_ASSERT_EQUAL_MEMORY(hash243_queue_at(&req->tails, 0),
                           hash243_queue_at(&req_de->tails, 0),
                           FLEX_TRIT_SIZE_243);
  TEST_ASSERT_EQUAL_MEMORY(hash243_queue_at(&req->tails, 1),
                           hash243_queue_at(&req_de->tails, 1),
                           FLEX_TRIT_SIZE_243);
...
static void test_response(void) {
  ...
  const char* json_false =
      "{\"state\":false,\"info\":\"" CONSISTENCY_INFO "\"}";

  check_consistency_res_t* res = check_consistency_res_new();
  char_buffer_t* serializer_out = char_buffer_new();

  serializer.vtable.check_consistency_deserialize_response(&serializer,
                                                           json_true, res);
  TEST_ASSERT_TRUE(res->state == true);
  TEST_ASSERT_NULL(res->info);

  serializer.vtable.check_consistency_serialize_response(&serializer, res,
                                                         serializer_out);
