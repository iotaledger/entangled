...
static void test_request(void) {
  ...
  ret = serializer.vtable.broadcast_transactions_serialize_request(
      &serializer, req, serializer_out);
  TEST_ASSERT_EQUAL_INT(RC_OK, ret);
  TEST_ASSERT_EQUAL_STRING(json_text, serializer_out->data);

  ret = serializer.vtable.broadcast_transactions_deserialize_request(
      &serializer, serializer_out->data, req_de);
  TEST_ASSERT_EQUAL_INT(RC_OK, ret);

  TEST_ASSERT_EQUAL_INT(hash_array_len(req->trytes),
                        hash_array_len(req_de->trytes));

  TEST_ASSERT_EQUAL_MEMORY(hash_array_at(req->trytes, 0),
                           hash_array_at(req_de->trytes, 0),
                           FLEX_TRIT_SIZE_8019);
