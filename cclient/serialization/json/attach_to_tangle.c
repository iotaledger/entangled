...
retcode_t json_attach_to_tangle_serialize_request(
  ...
  ret =
      flex_trits_to_json_string(json_root, kTrunk, obj->trunk, NUM_TRITS_HASH);
  if (ret != RC_OK) {
    goto done;
  }

  ret = flex_trits_to_json_string(json_root, kBranch, obj->branch,
                                  NUM_TRITS_HASH);
  if (ret != RC_OK) {
    goto done;
  }
...
retcode_t json_attach_to_tangle_serialize_response(
...
retcode_t json_attach_to_tangle_deserialize_request(
