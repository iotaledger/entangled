...
static inline retcode_t get_inclusion_state_req_hash_add(get_inclusion_state_req_t* const req,
                                                         flex_trit_t const* const hash) {
  return hash243_queue_push(&req->hashes, hash);
}
static inline flex_trit_t* get_inclusion_state_req_hash_get(get_inclusion_state_req_t* const req, size_t index) {
  return hash243_queue_at(&req->hashes, index);
}

static inline retcode_t get_inclusion_state_req_tip_add(get_inclusion_state_req_t* const req,
                                                        flex_trit_t const* const hash) {
  return hash243_queue_push(&req->tips, hash);
}
static inline flex_trit_t* get_inclusion_state_req_tip_get(get_inclusion_state_req_t* const req, size_t index) {
