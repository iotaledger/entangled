...
static inline retcode_t find_transactions_res_hashes_add(
    find_transactions_res_t* const res, flex_trit_t const* const hash) {
  return hash243_queue_push(&res->hashes, hash);
}
static inline flex_trit_t* find_transactions_res_hashes_get(
    find_transactions_res_t* const res, size_t index) {
