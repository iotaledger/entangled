...
static inline retcode_t get_balances_res_milestone_add(
    get_balances_res_t* const res, flex_trit_t const* const hash) {
  return hash243_queue_push(&res->milestone, hash);
}
static inline flex_trit_t* get_balances_res_milestone_get(
    get_balances_res_t* const res, size_t index) {
