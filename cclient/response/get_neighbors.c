...
neighbor_info_t* get_neighbors_res_create_neighbor(char_buffer_t* addr, int all_trans, int invalid_trans,
                                                   int new_trans) {
...
retcode_t get_neighbors_res_add_neighbor(get_neighbors_res_t* nbors, char_buffer_t* addr, int all_trans,
                                         int invalid_trans, int new_trans) {
  neighbor_info_t nb = {
      .address = addr, .all_trans_num = all_trans, .invalid_trans_num = invalid_trans, .new_trans_num = new_trans};
  if (!nbors) {
...
void get_neighbors_res_dump(get_neighbors_res_t* nbors) {
  ...
  for (nb = (neighbor_info_t*)utarray_front(nbors); nb != NULL; nb = (neighbor_info_t*)utarray_next(nbors, nb)) {
    ...
    printf(
        ...
        nb->address->data, nb->all_trans_num, nb->invalid_trans_num, nb->new_trans_num);
  }
}
