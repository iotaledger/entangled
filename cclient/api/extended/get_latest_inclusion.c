...
retcode_t iota_client_get_latest_inclusion(
  ...
  get_inclusion_state_req_t* inclusion_req = NULL;
  ...
  if (!node_info) {
    ...
    log_error(client_extended_logger_id,
              "%s create node info request failed: %s\n", __func__,
  ...
  inclusion_req = get_inclusion_state_req_new();
  if (!inclusion_req) {
    ret_code = RC_CCLIENT_OOM;
    log_error(client_extended_logger_id,
              "%s create get inclusion state request failed: %s\n", __func__,
              error_2_string(ret_code));
    goto done;
  }

  inclusion_req->hashes = transactions;
  ret_code = hash243_queue_push(&inclusion_req->tips,
