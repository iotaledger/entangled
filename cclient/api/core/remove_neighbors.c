...
retcode_t iota_client_remove_neighbors(const iota_client_service_t* const service,
  ...
  result = service->serializer.vtable.remove_neighbors_serialize_request(&service->serializer, req, req_buff);
  if (result != RC_OK) {
    goto done;
  }
  ...
  result = service->serializer.vtable.remove_neighbors_deserialize_response(&service->serializer, res_buff->data, res);
