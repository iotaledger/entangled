...
static void test_find_tx_empty(void) {
  ...
  TEST_ASSERT(iota_client_find_transactions(&g_serv, find_tx_req, find_tx_res) == RC_NULL_PARAM);
...
static void test_find_tx_by_bundle(void) {
  ...
  TEST_ASSERT(hash243_queue_push(&find_tx_req->bundles, flex_hash) == RC_OK);
  TEST_ASSERT(iota_client_find_transactions(&g_serv, find_tx_req, find_tx_res) == RC_OK);
...
static void test_find_tx_by_empty_bundle(void) {
  ...
  TEST_ASSERT(hash243_queue_push(&find_tx_req->bundles, flex_hash) == RC_OK);
  TEST_ASSERT(iota_client_find_transactions(&g_serv, find_tx_req, find_tx_res) == RC_CCLIENT_RES_ERROR);
  TEST_ASSERT_NULL(find_tx_res->hashes);
...
static void test_find_tx_by_address(void) {
  ...
  TEST_ASSERT(hash243_queue_push(&find_tx_req->addresses, flex_addr) == RC_OK);
  TEST_ASSERT(iota_client_find_transactions(&g_serv, find_tx_req, find_tx_res) == RC_OK);
  TEST_ASSERT_NOT_NULL(find_tx_res->hashes);
...
static void test_find_tx_by_tag(void) {
  ...
  TEST_ASSERT(flex_trits_from_trytes(flex_tag, NUM_TRITS_TAG, TEST_TAG_1, NUM_TRYTES_TAG, NUM_TRYTES_TAG) != 0);

  TEST_ASSERT(hash81_queue_push(&find_tx_req->tags, flex_tag) == RC_OK);
  TEST_ASSERT(iota_client_find_transactions(&g_serv, find_tx_req, find_tx_res) == RC_OK);
  TEST_ASSERT_NOT_NULL(find_tx_res->hashes);
...
static void test_find_tx_by_approvee(void) {
  ...
  TEST_ASSERT(hash243_queue_push(&find_tx_req->approvees, flex_hash) == RC_OK);
  TEST_ASSERT(iota_client_find_transactions(&g_serv, find_tx_req, find_tx_res) == RC_OK);
  TEST_ASSERT_NOT_NULL(find_tx_res->hashes);
