...
void s_init_curl(s_curl_t *const ctx) { memset(ctx->state, 0, sizeof(ctx->state)); }

void s_curl_absorb(s_curl_t *const ctx, trit_t const *const trits, size_t const length) {
  memcpy(ctx->state, trits, (length < S_HASH_LENGTH ? length : S_HASH_LENGTH) * sizeof(trit_t));
  transform(ctx);
  if (length <= S_HASH_LENGTH) {
    return;
  }
  s_curl_absorb(ctx, trits + S_HASH_LENGTH * sizeof(trit_t), length - S_HASH_LENGTH);
}

void s_curl_squeeze(s_curl_t *const ctx, trit_t *const trits, size_t const length) {
  memcpy(trits, ctx->state, (length < S_HASH_LENGTH ? length : S_HASH_LENGTH) * sizeof(trit_t));
  transform(ctx);
  if (length <= S_HASH_LENGTH) {
    return;
  }
  s_curl_squeeze(ctx, trits + S_HASH_LENGTH * sizeof(trit_t), length - S_HASH_LENGTH);
}
...
void sbox(s_curl_t *const c, s_curl_t *const s, size_t const i, size_t const j) {
  if (i < S_STATE_LENGTH) {
...
void s_curl_reset(s_curl_t *const ctx) { memset_safe(ctx->state, sizeof(ctx->state), 0, sizeof(ctx->state)); }
