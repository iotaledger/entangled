...
void test_bundle_hash(void) {
  ...
  flex_trits_from_trytes(address, NUM_TRITS_ADDRESS, TEST_ADDRESS_1,
                         NUM_TRYTES_ADDRESS, NUM_TRYTES_ADDRESS);

  flex_trit_t tag[FLEX_TRIT_SIZE_81];
  flex_trits_from_trytes(tag, NUM_TRITS_TAG, TEST_TAG, NUM_TRYTES_TAG,
                         NUM_TRYTES_TAG);

  flex_trit_t hash[FLEX_TRIT_SIZE_243];
  flex_trits_from_trytes(hash, NUM_TRITS_HASH, TEST_HASH, NUM_TRYTES_HASH,
                         NUM_TRYTES_HASH);
...
void test_value_out(void) {
  ...
  flex_trits_from_trytes(seed, NUM_TRITS_ADDRESS, TEST_SEED, NUM_TRYTES_ADDRESS,
                         NUM_TRYTES_ADDRESS);
  transfer_value_out_t OUTPUT = {seed, 2, 0};
  iota_transaction_t TX = {};
  flex_trit_t address_out[FLEX_TRIT_SIZE_243];
  flex_trits_from_trytes(address_out, NUM_TRITS_ADDRESS, TEST_ADDRESS_0,
                         NUM_TRYTES_ADDRESS, NUM_TRYTES_ADDRESS);

  flex_trit_t tag[FLEX_TRIT_SIZE_81];
  flex_trits_from_trytes(tag, NUM_TRITS_TAG, TEST_TAG_NULL, NUM_TRYTES_TAG,
                         NUM_TRYTES_TAG);
  transfer_t* transfer_out =
      transfer_value_out_new(&OUTPUT, tag, address_out, -1, 1548690545190);

  flex_trit_t address_in[FLEX_TRIT_SIZE_243];
  flex_trits_from_trytes(address_in, NUM_TRITS_ADDRESS, TEST_ADDRESS_1,
                         NUM_TRYTES_ADDRESS, NUM_TRYTES_ADDRESS);
  transfer_t* transfer_in =
      transfer_value_in_new(address_in, tag, 1, NULL, 0, 1548690545190);
  ...
  flex_trits_from_trytes(sig_buff, NUM_TRITS_SIGNATURE, TEST_SIG_1,
  ...
  tx = transfer_iterator_next(tf_iter);
  TEST_ASSERT_NOT_NULL(tx);
  flex_trits_from_trytes(sig_buff, NUM_TRITS_SIGNATURE, TEST_SIG_2,
                         NUM_TRYTES_SIGNATURE, NUM_TRYTES_SIGNATURE);
  TEST_ASSERT_EQUAL_MEMORY(sig_buff, tx->data.signature_or_message,
                           FLEX_TRIT_SIZE_6561);
  ...
  tx = transfer_iterator_next(tf_iter);
  TEST_ASSERT_NOT_NULL(tx);
  flex_trits_from_trytes(sig_buff, NUM_TRITS_SIGNATURE, TEST_SIG_3,
                         NUM_TRYTES_SIGNATURE, NUM_TRYTES_SIGNATURE);
  TEST_ASSERT_EQUAL_MEMORY(sig_buff, tx->data.signature_or_message,
                           FLEX_TRIT_SIZE_6561);
...
void test_transfer_data(void) {
  ...
  flex_trit_t address[FLEX_TRIT_SIZE_243];
  flex_trits_from_trytes(address, NUM_TRITS_ADDRESS, TEST_DATA_ADDRESS,
                         NUM_TRYTES_ADDRESS, NUM_TRYTES_ADDRESS);
