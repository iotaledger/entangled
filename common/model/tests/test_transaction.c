...
void test_read_write_transaction_obj(void) {
  ...
  TEST_ASSERT_EQUAL_MEMORY(transaction_obsolete_tag(&transaction),
                           TEST_FLEX_TRIT_OBSOLETE_TAG,
                           sizeof(TEST_FLEX_TRIT_OBSOLETE_TAG));
  TEST_ASSERT_EQUAL_INT64(transaction_timestamp(&transaction), TEST_TIMESTAMP);
  TEST_ASSERT_EQUAL_INT64(transaction_current_index(&transaction),
                          TEST_CURRENT_INDEX);
  TEST_ASSERT_EQUAL_INT64(transaction_last_index(&transaction),
                          TEST_LAST_INDEX);
  TEST_ASSERT_EQUAL_MEMORY(transaction_bundle(&transaction),
  ...
  TEST_ASSERT_EQUAL_MEMORY(transaction_branch(&transaction),
                           TEST_FLEX_TRIT_BRANCH,
                           sizeof(TEST_FLEX_TRIT_BRANCH));
  TEST_ASSERT_EQUAL_INT64(transaction_attachment_timestamp(&transaction),
                          TEST_ATTACHMENT_TIMESTAMP);
  TEST_ASSERT_EQUAL_INT64(transaction_attachment_timestamp_lower(&transaction),
                          TEST_ATTACHMENT_TIMESTAMP_LOWER);
  TEST_ASSERT_EQUAL_INT64(transaction_attachment_timestamp_upper(&transaction),
                          TEST_ATTACHMENT_TIMESTAMP_UPPER);
  TEST_ASSERT_EQUAL_MEMORY(transaction_nonce(&transaction),
                           TEST_FLEX_TRIT_NONCE, sizeof(TEST_FLEX_TRIT_NONCE));
