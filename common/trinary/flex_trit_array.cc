...
namespace trinary {
  FlexTritArray FlexTritArray::createFromTrits(const std::vector<trit_t> trits) {
    ...
    int8_to_flex_trit_array(flex_trits.data(), flex_len, trits.data(), trits.size(), trits.size());
  ...
  FlexTritArray FlexTritArray::createFromTrytes(const std::vector<tryte_t> trytes) {
    ...
    std::vector<flex_trit_t> flex_trits;
    flex_trits.resize(flex_len, 0);
    tryte_to_flex_trit(flex_trits.data(), num_trits, trytes.data(), trytes.size(), trytes.size());
  ...
  FlexTritArray FlexTritArray::slice(size_t start, size_t num_trits) {
    ...
    std::vector<flex_trit_t> flex_trits;
    flex_trits.resize(flex_len, 0);
    flex_trit_array_slice(flex_trits.data(), flex_len, flex_trits_.data(), num_trits_, start, num_trits);
  ...
  size_t FlexTritArray::insert(FlexTritArray &flex_trit_array, size_t start) {
    ...
    flex_trit_array_insert(flex_trits_.data(), num_trits_, flex_trit_array.flex_trits_.data(), flex_trit_array.num_trits_, start, flex_trit_array.num_trits_);
  ...
  std::vector<trit_t> FlexTritArray::trits() {
    ...
    flex_trit_array_to_int8(trits.data(), num_trits_, flex_trits_.data(), num_trits_, num_trits_);
    return trits;
  }
  ...
  std::vector<tryte_t> FlexTritArray::trytes() {
    ...
    flex_trit_to_tryte(trytes.data(), num_trytes, flex_trits_.data(), num_trits_, num_trits_);
    return trytes;
  }
}
}  // namespace iota
