...
static retcode_t prepare_statements(sqlite3_connection_t* const connection) {
  ...
  ret |= prepare_statement(connection->db, (sqlite3_stmt**)(&connection->statements.transaction_clear_metadata),
                           iota_statement_transaction_clear_metadata);
...
static retcode_t finalize_statements(sqlite3_connection_t* const connection) {
  ...
  ret |= finalize_statement(connection->statements.transaction_clear_metadata);
