...
static retcode_t prepare_statements(sqlite3_connection_t* const connection) {
  ...
  ret = prepare_statement(connection->db,
                          &connection->statements.transaction_insert,
                          iota_statement_transaction_insert);
  ret |= prepare_statement(connection->db,
                           &connection->statements.transaction_select_by_hash,
                           iota_statement_transaction_select_by_hash);
  ret |= prepare_statement(
      connection->db,
      &connection->statements.transaction_select_hashes_by_address,
      iota_statement_transaction_select_hashes_by_address);
  ret |= prepare_statement(
      connection->db,
      &connection->statements.transaction_select_hashes_of_approvers,
  ...
  ret |= prepare_statement(
      connection->db, &connection->statements.transaction_select_hashes_of_tips,
      iota_statement_transaction_select_hashes_of_tips);
  ret |= prepare_statement(
      connection->db,
      &connection->statements.transaction_select_hashes_of_milestone_candidates,
      iota_statement_transaction_select_hashes_of_milestone_candidates);
  ret |= prepare_statement(
      connection->db, &connection->statements.transaction_update_snapshot_index,
      iota_statement_transaction_update_snapshot_index);
  ret |= prepare_statement(
      connection->db, &connection->statements.transaction_update_solid_state,
      iota_statement_transaction_update_solid_state);
  ret |= prepare_statement(connection->db,
                           &connection->statements.transaction_exist,
                           iota_statement_transaction_exist);
  ret |= prepare_statement(connection->db,
                           &connection->statements.transaction_exist_by_hash,
                           iota_statement_transaction_exist_by_hash);
  ret |= prepare_statement(connection->db,
                           &connection->statements.transaction_approvers_count,
                           iota_statement_transaction_approvers_count);
  ret |= prepare_statement(connection->db,
                           &connection->statements.transaction_count,
                           iota_statement_transaction_count);
  ret |= prepare_statement(
      connection->db,
      &connection->statements.transaction_select_essence_and_metadata,
  ...
  ret |= prepare_statement(connection->db,
                           &connection->statements.milestone_select_by_hash,
                           iota_statement_milestone_select_by_hash);
  ret |= prepare_statement(connection->db,
                           &connection->statements.milestone_select_first,
                           iota_statement_milestone_select_first);
  ret |= prepare_statement(connection->db,
                           &connection->statements.milestone_select_last,
                           iota_statement_milestone_select_last);
  ret |= prepare_statement(connection->db,
                           &connection->statements.milestone_select_next,
                           iota_statement_milestone_select_next);
  ret |=
      prepare_statement(connection->db, &connection->statements.milestone_exist,
                        iota_statement_milestone_exist);
  ret |= prepare_statement(connection->db,
                           &connection->statements.milestone_exist_by_hash,
                           iota_statement_milestone_exist_by_hash);
  ret |= prepare_statement(connection->db,
                           &connection->statements.state_delta_store,
                           iota_statement_state_delta_store);
  ret |= prepare_statement(connection->db,
                           &connection->statements.state_delta_load,
...
static retcode_t finalize_statements(sqlite3_connection_t* const connection) {
  ...
  ret |= finalize_statement(
      connection->statements
          .transaction_select_hashes_of_transactions_to_request);
  ret |= finalize_statement(
      connection->statements.transaction_select_hashes_of_tips);
  ret |= finalize_statement(
      connection->statements.transaction_select_hashes_of_milestone_candidates);
  ret |= finalize_statement(
      connection->statements.transaction_update_snapshot_index);
  ret |=
      finalize_statement(connection->statements.transaction_update_solid_state);
...
retcode_t connection_init(void** const connection,
  ...
  if ((rc = sqlite3_exec(sqlite3_connection->db, sql, NULL, NULL, &err_msg)) !=
      SQLITE_OK) {
