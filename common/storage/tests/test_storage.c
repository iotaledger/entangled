...
void test_stored_transaction(void) {
  ...
  TEST_ASSERT(storage_transaction_load(&connection, TRANSACTION_FIELD_HASH, transaction_hash(test_tx), &pack) == RC_OK);
...
void test_transaction_update_solid_state(void) {
  ...
  TEST_ASSERT(storage_transaction_load(&connection, TRANSACTION_FIELD_HASH, transaction_hash(test_tx), &pack) == RC_OK);
  TEST_ASSERT_EQUAL_INT(1, pack.num_loaded);
  TEST_ASSERT(storage_transaction_update_solid_state(&connection, transaction_hash(test_tx), true) == RC_OK);
  hash_pack_reset(&pack);
  TEST_ASSERT(storage_transaction_load_metadata(&connection, transaction_hash(test_tx), &pack) == RC_OK);
  TEST_ASSERT_EQUAL_INT(1, pack.num_loaded);
...
void test_transactions_delete_two_transactions(void) {
  ...
  TEST_ASSERT(storage_transaction_exist(&connection, TRANSACTION_FIELD_HASH, transaction_hash(&second_test_transaction),
                                        &exist) == RC_OK);
  TEST_ASSERT(exist == true);
