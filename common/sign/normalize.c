...
void normalize_hash(trit_t const *const hash, byte_t *const normalized_hash) {
  for (int i = 0; i < SECURITY_LEVEL_MAX; i++) {
    ...
    for (int j = i * NORMALIZED_FRAGMENT_LENGTH;
      ...
      normalized_hash[j] =
          (hash[j * TRYTE_WIDTH] + hash[j * TRYTE_WIDTH + 1] * 3 +
           hash[j * TRYTE_WIDTH + 2] * 9);
...
void normalize_hash_to_trits(trit_t const *const hash,
...
void normalize_flex_hash(flex_trit_t const *const hash,
...
void normalize_flex_hash_to_trits(flex_trit_t const *const hash,
                                  trit_t *const normalized_hash) {
  byte_t normalized_bundle_bytes[HASH_LENGTH_TRYTE];
