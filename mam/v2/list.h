...
#define def_mam2_list_node(info_type, node_name) \
...
#define def_mam2_list(node_name, list_name) \
  typedef struct _##list_name {             \
    node_name *begin, *end;                 \
  } list_name

#define mam2_list_node_insert_next(_this, _new) \
  do {                                          \
    MAM2_ASSERT(_this);                         \
    MAM2_ASSERT(_new);                          \
...
#define mam2_list_node_insert_prev(_this, _new) \
  do {                                          \
    MAM2_ASSERT(_this);                         \
    MAM2_ASSERT(_new);                          \
...
#define mam2_list_node_remove_this(_this)             \
  do {                                                \
    MAM2_ASSERT(_this);                               \
...
#define mam2_list_insert_end(list, node)          \
  do {                                            \
    if (0 == list.begin) {                        \
      ...
      MAM2_ASSERT(0 != list.end);                 \
      mam2_list_node_insert_next(list.end, node); \
    }                                             \
    list.end = node;                              \
  } while (0)

#define mam2_list_remove(list, node)  \
  do {                                \
    MAM2_ASSERT(node);                \
    if (node == list.begin) {         \
    ...
    mam2_list_node_remove_this(node); \
  } while (0)
