 ...
 */
#define MAM2_SPONGE_SET_CONTROL_TRYTE(S, i, c0, c1, c2)                   \
  trits_put3(trits_drop(sponge_state_trits(S), MAM2_SPONGE_RATE + 3 * i), \
             tryte_from_trits(c0, c1, c2))

/*! \brief Test `i`-th control trit equals zero? */
...
MAM2_SAPI void sponge_absorb(isponge *s, trit_t c2, trits_t X) {
  ...
  do {
    ...
    if (!MAM2_SPONGE_IS_CONTROL_TRIT_ZERO(s, 1)) {
      MAM2_SPONGE_SET_CONTROL_TRYTE(s, 1, c0, c1, c2);
      sponge_transform(s);
    }

    trits_copy_pad10(Xi, sponge_outer1_trits(s));
    MAM2_SPONGE_SET_CONTROL_TRYTE(s, 0, c0, c1, c2);
