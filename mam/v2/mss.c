...
err_t mss_create(mss_t *mss, mss_mt_height_t d) {
  ...
  do {
...
#if defined(MAM2_MSS_TRAVERSAL)
    mss->auth_path = mam2_words_alloc(MAM2_MSS_MT_AUTH_WORDS(d));
    err_guard(mss->auth_path, err_bad_alloc);

    // add 1 extra hash for dirty hack (see mss.c)
    mss->hashes = mam2_words_alloc(MAM2_MSS_MT_HASH_WORDS(d, 1));
    ...
    err_guard(mss->nodes, err_bad_alloc);
#else
    mss->merkle_tree = mam2_words_alloc(MAM2_MSS_MT_WORDS(d));
...
#if defined(MAM2_MSS_TRAVERSAL)
  if (mss->auth_path) {
    mam2_words_free(mss->auth_path), mss->auth_path = 0;
  }
  if (mss->hashes) {
    mam2_words_free(mss->hashes), mss->hashes = 0;
  }
  if (mss->nodes) {
...
#else
  if (mss->merkle_tree) {
    mam2_words_free(mss->merkle_tree);
    mss->merkle_tree = 0;
  }
#endif
}
