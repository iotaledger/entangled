...
void poly_ntt(poly_t const f, poly_t t) {
  ...
  for (size_t i = MAM2_POLY_N_LOG; i--;) {
    for (size_t j = 0; j < MAM2_POLY_N / 2; ++j) {
      r = j & ~(((size_t)1 << i) - 1);
      /*r = j - (j % (1 << i));*/
      c = u[j + j];
    ...
    for (size_t j = 0; j < MAM2_POLY_N; ++j) {
      u[j] = t[j];
    }
  }
}
...
bool poly_from_trits(poly_t f, trits_t t) {
  ...
  for (i = 0; i < MAM2_POLY_N; ++i, t = trits_drop(t, 9)) {
    c = trits_get9(t);
    if ((c < -((MAM2_POLY_Q - 1) / 2)) || (((MAM2_POLY_Q - 1) / 2) < c)) {
      break;
    }
...
trint9_t poly_coeff_to_trint9(poly_coeff_t c) {
  ...
  t = (MAM2_POLY_Q - 1) / 2 < c ? (trint9_t)c - MAM2_POLY_Q : c;
