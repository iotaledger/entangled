...
static void test_psks_serialization(void) {
  ...
  tryte_t pre_shared_key[MAM2_PSK_KEY_SIZE / 3];

  for (size_t i = 0; i < 26; i++) {
    memset(id, 'A' + i, MAM2_PSK_ID_SIZE / 3);
    trytes_to_trits(id, psk.id, MAM2_PSK_ID_SIZE / 3);
    memset(pre_shared_key, 'A' + i, MAM2_PSK_KEY_SIZE / 3);
    trytes_to_trits(pre_shared_key, psk.key, MAM2_PSK_KEY_SIZE / 3);
    TEST_ASSERT(mam_psk_t_set_add(&psks_1, &psk) == RC_OK);
  }

  size_t size = mam_psks_serialized_size(psks_1);

  TEST_ASSERT_EQUAL_INT(size, 26 * (MAM2_PSK_ID_SIZE + MAM2_PSK_KEY_SIZE));
