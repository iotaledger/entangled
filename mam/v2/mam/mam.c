...
retcode_t mam_recv_packet(mam_recv_packet_context_t *cfg, trits_t *b,
  ...
  if (trits_is_null(*payload)) {
    ...
    ERR_GUARD_GO_TO(trits_size(*payload) <= pb3_sizeof_ntrytes(sz),
                    RC_MAM2_BUFFER_TOO_SMALL, e, cleanup);
    p = pb3_trits_take(payload, pb3_sizeof_ntrytes(sz));
  }
  ERR_GUARD_GO_TO(pb3_unwrap_crypt_ntrytes(s, b, p), e, cleanup);

  /*  absorb oneof checksum */
  ERR_GUARD_GO_TO(pb3_unwrap_absorb_tryte(s, b, &checksum), e, cleanup);

  if (mam_msg_checksum_none == checksum) {
    /*    absorb null none = 0; */
    ERR_GUARD_GO_TO(mam_unwrap_checksum_none(s, b), e, cleanup);
  } else if (mam_msg_checksum_mac == checksum) {
    /*    MAC mac = 1; */
    ERR_GUARD_GO_TO(mam_unwrap_checksum_mac(s, b), e, cleanup);
  } else if (mam_msg_checksum_mssig == checksum) {
    /*    MSSig mssig = 2; */
    ERR_GUARD_GO_TO(
        mam_unwrap_checksum_mssig(s, b, cfg->spongos_mss, cfg->spongos_wots,
                                  cfg->pk, cleanup),
        e);
  } else {
    ERR_GUARD_GO_TO(0, RC_MAM2_PB3_BAD_ONEOF, e, cleanup);
  }
