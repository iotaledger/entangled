...
static retcode_t mam_api_bundle_read_packet_from_msg(mam_api_t *const api, mam_msg_read_context_t *ctx, trits_t msg,
...
retcode_t mam_api_init(mam_api_t *const api, tryte_t const *const mam_seed) {
  ...
  api->ntru_pks = NULL;
  api->psks = NULL;
  ERR_BIND_RETURN(trit_t_to_mam_msg_write_context_t_map_init(&api->write_ctxs, MAM_MSG_ID_SIZE), ret);
  ERR_BIND_RETURN(trit_t_to_mam_msg_read_context_t_map_init(&api->read_ctxs, MAM_MSG_ID_SIZE), ret);
  api->channels = NULL;
  api->channel_ord = 0;
...
retcode_t mam_api_add_trusted_channel_pk(mam_api_t *const api, tryte_t const *const pk) {
  ...
  trytes_to_trits(pk, chid.key, MAM_CHANNEL_ID_SIZE / NUMBER_OF_TRITS_IN_A_TRYTE);
...
retcode_t mam_api_add_trusted_endpoint_pk(mam_api_t *const api, tryte_t const *const pk) {
  ...
  trytes_to_trits(pk, epid.key, MAM_CHANNEL_ID_SIZE / NUMBER_OF_TRITS_IN_A_TRYTE);
...
retcode_t mam_api_create_channel(mam_api_t *const api, size_t const height, tryte_t *const channel_id) {
  ...
  memcpy(channel_pk.key, trits_begin(mam_channel_id(&channel)), MAM_CHANNEL_ID_SIZE);
  ERR_BIND_RETURN(mam_pk_t_set_add(&api->trusted_channel_ids, &channel_pk), ret);
  trits_to_trytes(trits_begin(mam_channel_id(&channel)), channel_id, MAM_CHANNEL_ID_SIZE);
  api->channel_ord++;
...
mam_channel_t *mam_api_get_channel(mam_api_t const *const api, tryte_t const *const channel_id) {
  ...
  SET_ITER(api->channels, entry, tmp) {
    if (memcmp(trits_begin(mam_channel_id(&entry->value)), channel_id_trits, MAM_CHANNEL_ID_SIZE) == 0) {
      return &entry->value;
    }
  }
...
retcode_t mam_api_create_endpoint(mam_api_t *const api, size_t const height, tryte_t const *const channel_id,
  ...
  ERR_BIND_RETURN(mam_pk_t_set_add(&api->trusted_endpoint_ids, &endpoint_pk), ret);
  ...
  *is_last_packet = false;
  ...
  if (ord == 0) {
    ...
    ERR_BIND_RETURN(mam_msg_read_header(&ctx, &msg, api->psks, api->ntru_sks, trits_from_rep(MAM_MSG_ID_SIZE, tag),
                                        &api->trusted_channel_ids, &api->trusted_endpoint_ids),
                    ret);
    ...
    if (packet_index < bundle_transactions_size(bundle)) {
      ...
      ERR_BIND_RETURN(mam_api_bundle_read_packet_from_msg(api, &ctx, msg, payload, payload_size, is_last_packet), ret);
    }
...
static retcode_t mam_pks_serialize(mam_pk_t_set_t const pks, trits_t *const trits) {
  ...
  SET_ITER(pks, entry, tmp) { pb3_encode_ntrytes(trits_from_rep(MAM_CHANNEL_ID_SIZE, entry->value.key), trits); }
...
retcode_t mam_pks_deserialize(trits_t *const trits, mam_pk_t_set_t *const pks) {
  ...
  for (size_t i = 0; i < pks_size; i++) {
    ERR_BIND_RETURN(pb3_decode_ntrytes(trits_from_rep(MAM_CHANNEL_ID_SIZE, pk.key), trits), ret);
...
size_t mam_api_serialized_size(mam_api_t const *const api) {
  return mam_prng_serialized_size() + mam_ntru_sks_serialized_size(api->ntru_sks) +
         ...
         mam_api_read_ctx_map_serialized_size(&api->read_ctxs) + mam_channels_serialized_size(api->channels) +
         pb3_sizeof_longtrint() + mam_pks_serialized_size(api->trusted_channel_ids) +
...
retcode_t mam_api_deserialize(trits_t *const buffer, mam_api_t *const api) {
  ...
  ERR_BIND_RETURN(mam_api_write_ctx_map_deserialize(buffer, &api->write_ctxs), ret);
  ERR_BIND_RETURN(mam_api_read_ctx_map_deserialize(buffer, &api->read_ctxs), ret);
  ERR_BIND_RETURN(mam_channels_deserialize(buffer, &api->prng, &api->channels), ret);
  ...
  SET_ITER(api->write_ctxs.map, curr_ctx_entry, tmp_ctx_entry) {
    SET_ITER(api->channels, curr_channel_entry, tmp_channel_entry) {
      if (memcmp(curr_channel_entry->value.mss.root, curr_ctx_entry->value.mss_root, MAM_CHANNEL_ID_SIZE) == 0) {
        curr_ctx_entry->value.mss = &curr_channel_entry->value.mss;
      } else {
        SET_ITER(curr_channel_entry->value.endpoints, endpoint_entry, tmp_endpoint_entry) {
          if (memcmp(endpoint_entry->value.mss.root, curr_ctx_entry->value.mss_root, MAM_ENDPOINT_ID_SIZE) == 0) {
...
retcode_t mam_api_load(char const *const filename, mam_api_t *const api) {
  ...
  if (trits_is_null(trits = trits_alloc(bytes_size * NUMBER_OF_TRITS_IN_A_BYTE))) {
    ret = RC_OOM;
    goto done;
  }

  bytes_to_trits(bytes, bytes_size, trits.p + trits.d, bytes_size * NUMBER_OF_TRITS_IN_A_BYTE);
