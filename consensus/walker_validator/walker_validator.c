...
retcode_t iota_consensus_walker_validator_init(walker_validator_t *impl,
...
retcode_t iota_consensus_walker_validator_destroy(walker_validator_t *impl) {
  logger_helper_destroy(WALKER_VALIDATOR_LOGGER_ID);
  impl->tangle = NULL;
  impl->milestone = NULL;
  impl->lv = NULL;
}
retcode_t iota_consensus_walker_validator_is_valid(
    const walker_validator_t *impl, trit_array_p tx_hash) {
